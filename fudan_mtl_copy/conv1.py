...
def conv1(input,scope):
    with tf.variable_scope(scope,reuse = tf.AUTO_REUSE):
        ...
        #[batch,T,1,20]
        output = tf.nn.conv2d(input = input,filter=tf.Variable(tf.random_normal([3,3,1,200])),strides = [1,1,3,1],padding = "SAME")
        #[batch,T,20,1]
...
def max_pool(input,scope):
    with tf.variable_scope(scope, reuse=None):
        ...
        input = tf.expand_dims(input,-1)
        #[batch,T,1,1]
        output = tf.nn.max_pool(value=input,ksize=[1,1,200,1],strides=[1,1,200,1],padding="VALID",name ="padding")
