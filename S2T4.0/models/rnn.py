...
class rnn_decoder(nn.Module):
    def __init__(self, config, vocab_size, label_tree, embedding=None, score_fn=None, use_cuda = False):
        ...
        self.init_state_linear_in = nn.Linear(2*config.decoder_hidden_size, config.decoder_hidden_size)
        # self.init_state_linear_out = nn.Linear(config.decoder_hidden_size+config.emb_size, config.decoder_hidden_size)
        self.init_state_linear_out = nn.Linear(2*config.decoder_hidden_size, config.decoder_hidden_size)
    ...
    def forward(self, inputs, init_state, contexts, label_tree, input_lens,use_cuda = True):
        ...
        if not self.config.global_emb:
            ...
            rlevel = inputs[0]
            level_outputs = []            
            embs = self.embedding(rlevel[:, :-1])  # batch, nlabels, emb_size
            for emb in embs.split(1, dim=1):  # batch, 1, emb_size（每个位置）
                output, state = self.rnn(emb.squeeze(1), init_state)  # 以该位置的embed， 和上一位置的hidden state作为输入
                # batch * hidden_size
                output, attn_weights = self.attention(output, contexts)  # 以output为key，得到Context的weighted sum，
                output = self.dropout(output)
                level_outputs += [output]
            phidden= torch.stack(level_outputs)  # nlabels * batch * size
            phidden = phidden.transpose(0, 1)#batch*nlabels*size

            for i in range(0,len(inputs)-1):
                plen = input_lens[i] #[len of each sample at this level, ]
                level = inputs[i + 1]  # batch * nlabels
                # batch, nlabels
                mask = torch.zeros(len(phidden), len(phidden[0]))
                for idx, l in enumerate(plen):
                    # mask out <s> and </s> and padding.（置0）
                    mask[idx][1:l.data[0]-1] = 1

                # batch, nlabels, hidden_size
                if(use_cuda):
                    phidden = phidden.mul(Variable(mask.unsqueeze(2)).cuda())#为什么要把padding和<s>mask掉，因为要给下一层使用
                else:
                    phidden = phidden.mul(Variable(mask.unsqueeze(2)))

                state = self.init_state(phidden, contexts, init_state) # both of decoder_hidden_size
                ...
                phidden = level_outputs.transpose(0, 1)
    ...
    def init_state(self, hiddens, context, init_state, activation="tanh"):
        # hiddens: batch, nlabels, hidden_size
        # context: batch, seq_len, hidden_size
        # init_state: (num_layers * num_directions, batch, hidden_size)
        num_layers = init_state[0].size(0)
        x = torch.sum(hiddens, dim=1) # batch, hidden_size（把此层的hidden都加起来做为初始s0？）
        x = x.expand(num_layers, -1, -1) # num_layers, batch, emb_size
        #print(torch.cat([init_state[0],x],-1).shape)
    ...
    def tree_sample(self, src, label_tree, contexts, init_state, level_mask = True,max_len=25,use_cuda = True):
        ...
        # root level
        rlevel = torch.zeros(batch,3)#batch, seq,
        rlevel[:,0] = label_tree.label2idx['<s>']
        rlevel[:,1] = label_tree.label2idx['root']
        rlevel[:,2] = label_tree.label2idx['</s>']
        ...
        #other level
        for j in range(len(label_tree.levels)-1):

            mask = torch.zeros(len(phidden), len(phidden[0]))
            for idx in range(batch):
                # mask out <s> 预测的时候如果找</s>太麻烦，现在就减化成只mask <s>
                mask[idx][1:] = 1
            # batch, nlabels, hidden_size
            if (use_cuda):
                phidden = phidden.mul(Variable(mask.unsqueeze(2)).cuda())  # 为什么要把padding和<s>mask掉，因为要给下一层使用
           ...
           # count = torch.zeros(batch)  # to count whether all samples meet the end.
            for i in range(self.config.max_tgt_len):
               # a = sum(count)
                ...
                #level_outputs是本层预测出的所有label（25个)
                if type(level_output) == type(None):#还没有预测出label
            ...
            outputs.append(level_output)
            # batch, nlabels
            plevel = torch.stack(plevel)  # nlabels * batch * size
            phidden = plevel.transpose(0, 1)#batch*nlabels*size
            #plevel = level_output
