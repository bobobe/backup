...
from data.Label_tree import Label_tree
...
def makeData(srcFile, tgtFile, srcDicts, tgtDicts, sort=False, char=False, label_tree=None):
    src, tgt = [], [] 
    raw_src, raw_tgt = [], [] 
    sizes = [] 
    count, ignored = 0, 0
    ...
    while True: 
        ...
        if opt.src_length == 0 or (len(srcWords) <= opt.src_length and len(tgtWords) <= opt.tgt_length):
            ...
            if char:
                ...
                srcWords = [word+" " for word in srcWords] # why add a blank.
            ...
            levels, levels_id = label_tree.sample_tree(tgtWords)
            tgt += [levels_id]
    ...
    # tgt: nsamples, nlevels, nlabels of torch tensor.
    return dataset(src, tgt, raw_src, raw_tgt) 


def main():
    label_tree = Label_tree("data/rcv1.topics.hier.orig.txt")
    ...
    train = makeData(opt.train_src, opt.train_tgt, dicts['src'], dicts['tgt'], char=opt.char, label_tree=label_tree)

    print('Preparing validation ...')
    valid = makeData(opt.valid_src, opt.valid_tgt, dicts['src'], dicts['tgt'], char=opt.char, label_tree=label_tree)

    print('Preparing test ...')
    test = makeData(opt.test_src, opt.test_tgt, dicts['src'], dicts['tgt'], char=opt.char, label_tree=label_tree)
