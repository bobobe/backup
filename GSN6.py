...
class GSN(object):
    ...
    def build_graph(self):
        ...
        #lstm shared encoder
        lstm_output = self.lstm_encoder(target_embeddings,"target")
        rel_loss = self.relation_loss(lstm_output,"target_lstm")
        self.train_accuracy = self.accuracy(lstm_output, "train")
        self.loss = rel_loss
        self.train_op = self.trainstep_op(self.loss)
        tf.summary.scalar("loss", self.loss)
        #dev
        lstm_output = self.lstm_encoder(target_embeddings,"target")
        rel_loss = self.relation_loss(lstm_output, "dev")
        self.dev_loss = rel_loss
        self.labels_predict = tf.cast(tf.argmax(lstm_output, axis=-1), tf.int32)
        self.dev_accuracy = self.accuracy(lstm_output, "dev")
    ...
    def add_placeholders(self):
        ...
        self.source_seq_len = tf.placeholder(tf.int32, shape=[None], name="source_seq_len")
        self.target_seq_len = tf.placeholder(tf.int32, shape=[None], name="target_seq_len")
    ...
    def lstm_encoder(self,origin_embedding_input,scope):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            cell_fw = LSTMCell(150, reuse=tf.AUTO_REUSE)
            cell_bw = LSTMCell(150, reuse=tf.AUTO_REUSE)
            h_fw,h_bw=None,None
            if(scope=="source"):
                    _, ((_, h_fw), (_, h_bw)) = tf.nn.bidirectional_dynamic_rnn(
                    cell_fw=cell_fw,
                    cell_bw=cell_bw,
                    inputs=origin_embedding_input,
                    dtype=tf.float32,
                    sequence_length=self.source_seq_len)
            elif(scope=="target"):
                    _, ((_, h_fw), (_, h_bw)) = tf.nn.bidirectional_dynamic_rnn(
                    cell_fw=cell_fw,
                    cell_bw=cell_bw,
            ...
            #[batch,150]:h_fw
            # print(h_bw.get_shape())
            # print(h_fw.get_shape())
            output = tf.concat([h_fw, h_bw], axis=-1)#[batch,300]
            output = tf.nn.dropout(output, self.dropout_kp)
    ...
    def deconvolution_layer(self,shared_conv_output_before_pool,private_conv_output_before_pool,scope,reuse = None):
        ...
        with tf.variable_scope(scope,reuse = reuse):
            ...
            deconv_output = tf.nn.conv2d_transpose(ps, filter=tf.get_variable(name="filters", shape=[3, self.all_emb_size, 1, self.num_filters],initializer=tf.contrib.layers.xavier_initializer(),
                            dtype=tf.float32), output_shape=[tf.shape(ps)[0], self.max_seq_len, self.all_emb_size, 1],
    ...
    def train(self, train, dev, test):#训练集和验证集and test
        """
        ...
        with tf.Session(config = self.config) as sess:
            ...
            for epoch in range(self.epoch_num):
                self.run_one_epoch(sess, train, dev, test, epoch, saver)

    # 一个epoch会分批训练所有样本
    def run_one_epoch(self, sess, train, dev, test, epoch, saver):
        """
        ...
        for step, (source_batch, target_batch) in enumerate(batches):
            ...
            source_x, source_y, source_seq_len = zip(*source_batch)
            target_x, target_y, target_seq_len = zip(*target_batch)
            source_domain = np.array([[1,0]]*len(source_x))
            target_domain = np.array([[0,1]]*len(target_x))

            #source train
            step_num = epoch * num_batches + step + 1
            source_feed_dict = {self.target_word_ids: target_x,
                                self.target_seq_len: target_seq_len,
                                self.relation_labels: target_y,
                                self.dropout_kp: self.keep_prob}
            _, loss,train_accuracy, summary_target, step_num_ = sess.run([self.train_op, self.loss,
                                                                                self.train_accuracy, self.target_merged,
                                                                                self.global_step],
            ...
            if(step + 1 == 1 or (step + 1) % 10 == 0 or step + 1 == num_batches):#log every 10 batch_steps
                self.logger.info(
                '{} epoch {}, batch_step {}/{}, loss: {:.4}, train_acccuracy: {:.4}, global_step: {}'
                .format(start_time, epoch + 1, step + 1,num_batches, loss, train_accuracy, step_num))

        #dev after every epoch
        self.logger.info('===========validation / test===========')
        # dev
        f1,accuracy = self.dev_one_epoch(sess, test, epoch)
        if f1 > self.best_f1:
            self.best_f1 = f1
            saver.save(sess, self.model_path + "-f1:{:.5g}".format(self.best_f1), global_step=epoch)  # 按训练迭代轮次存储。
    ...
    def dev_one_epoch(self, sess, dev, epoch):
        ...
        source_dev = dev[0]
        target_dev = dev[1]
        dev_x, dev_y, dev_seq_len = zip(*target_dev)
        dev_feed_dict = {self.target_word_ids: dev_x,
                         self.target_seq_len: dev_seq_len,
        ...
        self.logger.info('epoch {},  dev_loss: {:.4}, dev_accuracy: {:.4},best_dev_accuracy: {:.4},macro_f1: {:.4}'
                         .format(epoch + 1, dev_loss, dev_accuracy, self.best_accuracy,macro_f1))
        return macro_f1, dev_accuracy
